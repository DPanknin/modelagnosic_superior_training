...
def prediction_strategy(train_inputs, train_prior_dist, train_labels, likelihood, jitter = None, cholJitter = None): # TODO added by DANNY (jitter)
    ...
    return cls(train_inputs, train_prior_dist, train_labels, likelihood, jitter=jitter, cholJitter=cholJitter)


class DefaultPredictionStrategy(object):
    def __init__(self, train_inputs, train_prior_dist, train_labels, likelihood, root=None, inv_root=None, jitter = None, cholJitter = None):
        # Get training shape
    ...
    def mean_cache(self):
        ...
        # TODO the above mean_cache estimate is too bad for SGDML
        if True:
            L = psd_safe_cholesky(delazify(train_train_covar.evaluate_kernel()), jitter = self.cholJitter, max_tries=100) # TODO added by DANNY
