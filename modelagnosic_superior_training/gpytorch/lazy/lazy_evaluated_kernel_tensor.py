...
class LazyEvaluatedKernelTensor(LazyTensor):
    ...
    def evaluate_kernel(self):
        ...
        if settings.debug.on(): # TODO changed by DANNY (treatment of sparse kernel list representation)
            if isinstance(res, list):
                ...
                for ll in res:
                    s1 += ll.shape[0]
                    s2 += ll.shape[1]
                if (s1 != self.shape[0] or s2 != self.shape[1]) and (self.shape[0] != res[1].shape[0] + res[2].shape[0] or self.shape[1] != res[1].shape[1] + res[2].shape[1]):
                    raise RuntimeError(
    ...
    @cached
    def evaluate(self): # TODO changed by DANNY
        #return self.evaluate_kernel().evaluate()
        k = self.evaluate_kernel()
        if isinstance(k, list):
            return [l.evaluate() for l in k]
        else:
            return k.evaluate()
