...
class LazyTensor(ABC):
    ...
    * :func:`~gpytorch.lazy.LazyTensor._check_args`, which performs error checking on the arguments supplied to the
    ...
    def _cholesky(self, upper=False, jitter = None): # TODO changed by DANNY (jitter,max_tries)
        """
        ...
        cholesky = psd_safe_cholesky(evaluated_mat.type(_linalg_dtype_cholesky.value()), upper=upper, jitter = jitter, max_tries=100).contiguous()
    ...
    def cholesky(self, upper=False, jitter = None): # TODO changed by DANNY (jitter)
        """
        ...
        """
        chol = self._cholesky(upper=False, jitter = jitter)
        if upper:
