...
class VariationalStrategy(_VariationalStrategy):
    ...
    def _cholesky_factor(self, induc_induc_covar, jitter = None):
        L = psd_safe_cholesky(delazify(induc_induc_covar).type(_linalg_dtype_cholesky.value()), jitter = jitter, max_tries=100) # TODO changed by DANNY
    ...
    def forward(self, x, inducing_points, inducing_values, variational_inducing_covar=None, **kwargs):
        ...
        if not _is_in_cache_ignore_args(self, "cholesky_factor"):
            ...
            induc_induc_covar = delazify(induc_induc_covar)
        ...
        # if predictionsOnly:
        if False:
            predictive_mean = (induc_data_covar.transpose(-1, -2).type(_linalg_dtype_cholesky.value()) @ L.transpose(-1, -2).inv_matmul(inducing_values.unsqueeze(-1).type(_linalg_dtype_cholesky.value()))).squeeze(-1).to(full_inputs.dtype) + test_mean
        else:
